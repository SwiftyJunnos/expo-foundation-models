...
- Guard iOS 27-only tool calling options in both tool generation paths so the
  module builds with the iOS 27 SDK while retaining iOS 26 deployment support.
- Correct the streamed tool-call completion closure to accept parsed JSON objects
  and explicitly capture the validation method's receiver.
- Adopt the UIKit scene lifecycle in the TestFlight example so iOS 27 SDK builds
  launch successfully, preserving Expo lifecycle callbacks and React Native links.
